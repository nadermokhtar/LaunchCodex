class Api::V1::ActionsController < ApplicationController
  ...
  def update_for_tomorrow
    Action.update(params[:action_id], date_due: Date.today + 1)
    render json: prepare_actions
  end
  ...
  def prepare_actions
    today = current_user.actions.where(date_due: Date.today).order('priority desc').order('created_at desc')
    yesterday = current_user.actions.where(date_due: Date.today - 1).order('priority desc').order('created_at desc')
    tomorrow = current_user.actions.where(date_due: Date.today + 1).order('priority desc').order('created_at desc')
    future = current_user.actions.where(date_due: nil).order('priority desc').order('created_at desc')
    events = current_user.actions.where(category: 'event').order('date_due asc')
    { todays_actions: today, yesterday_actions: yesterday, future_actions: future, all_events: events, tomorrow_actions: tomorrow }
  end
